...
def process_schedule(file_path):
    ...
    employees = pd.read_excel(file_path, 'Employees').to_dict('records')
    Positions = pd.read_excel(file_path, 'Positions and Shifts')
    TimeOff = pd.read_excel(file_path, 'Time off requests').to_dict('records')
    Attendance = pd.read_excel(file_path, 'Attendance').to_dict('records')
    WeeklyInfo = pd.read_excel(file_path, 'Weekly Info')
    ...
    workbook.save(f"{CompanyName} Schedule {MondayDate2} to {SundayDate2}.xlsx")
