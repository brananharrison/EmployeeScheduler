...
Let {X<sub>21</sub>, X<sub>2</sub>} be 2 employees' comparison scores on the interval [0,1]. Without loss of generality suppose X1 > X2. Define Z = 5*(Xsub>21</sub> - Xsub>22</sub> ) as the difference between the scores with a coefficient of 5 in preparation for a composition with the sigmoid function. The coefficient is chosen by the manager, with a higher coefficient resulting in more skew towards the leader.Finally we utilize the function below to decide the probability of the leader being chosen. Greater differences in comparison scores yield a higher probability of the leader being chosen.
